...
ErrStatus exmc_synchronous_dynamic_ram_init(uint32_t sdram_device)
{
    ...
    /* 64ms, 8192-cycle(8192 13bit地址线 2^13) refresh, 64ms/8192=7.81us */
...
/**
    ...
    \param[in]  numtowrite: number of bytes to write
    \param[out] none
    \retval     none
*/
...
/**
    ...
    \param[in]  pbuffer: pointer to buffer
    \param[in]  writeaddr: SDRAM memory internal address from which the data will be written
    \param[in]  numtowrite: number of half-word to write
    \param[out] none
    \retval     none
*/
...
/**
    ...
    \param[in]  pbuffer: pointer to buffer
    \param[in]  readaddr: SDRAM memory internal address to read from
    \param[in]  numtoread: number of half-word to read
    \param[out] none
    \retval     none
*/
...
/**
    \brief      write a byte data(data is 8 bits) to the EXMC SDRAM memory
    \param[in]  sdram_device: specify which a SDRAM memory block is written
    \param[in]  pbuffer: pointer to buffer
    \param[in]  writeaddr: SDRAM memory internal address from which the data will be written
    \param[in]  numtowrite: number of bytes to write
...
void sdram_set_8(uint32_t sdram_device, uint8_t data, uint32_t writeaddr, uint32_t numtowrite)
{
    ...
    for (; numtowrite != 0; numtowrite--) {
        /* Transfer data to the memory */
        *(uint8_t *)(temp_addr + writeaddr) = data;
...
/**
    \brief      write a half-word data(data is 16 bits) to the EXMC SDRAM memory
    \param[in]  sdram_device: specify which a SDRAM memory block is written
...
void sdram_set_16(uint32_t sdram_device, uint16_t data, uint32_t writeaddr, uint32_t numtowrite)
{
    ...
    for (; numtowrite != 0; numtowrite--) {
        /* Transfer data to the memory */
        *(uint16_t *)(temp_addr + writeaddr) = data;
...
/**
    \brief      write 3 byte data(data is 24 bits) to the EXMC SDRAM memory
    \param[in]  sdram_device: specify which a SDRAM memory block is written
    \param[in]  data: write data
    \param[in]  writeaddr: SDRAM memory internal address from which the data will be written
    \param[in]  numtowrite: number of 3 byte to write
    \param[out] none
    \retval     none
*/
void sdram_set_24(uint32_t sdram_device, uint32_t data, uint32_t writeaddr, uint32_t numtowrite)
{
    ...
    for (; numtowrite != 0; numtowrite--) {
        /* Transfer data to the memory */
        *(uint16_t *)(temp_addr + writeaddr) = data & 0xffff;

        /* Increment the address */
        writeaddr += 2;

        /* Transfer data to the memory */
        *(uint8_t *)(temp_addr + writeaddr) = (data >> 16) & 0xff;
...
/**
    \brief      write a word data(data is 32 bits) to the EXMC SDRAM memory
    \param[in]  sdram_device: specify which a SDRAM memory block is written
    \param[in]  data: write data
    \param[in]  writeaddr: SDRAM memory internal address from which the data will be written
    \param[in]  numtowrite: number of word to write
    \param[out] none
    \retval     none
*/
void sdram_set_32(uint32_t sdram_device, uint32_t data, uint32_t writeaddr, uint32_t numtowrite)
{
    ...
    for (; numtowrite != 0; numtowrite--) {
        ...
        *(uint16_t *)(temp_addr + writeaddr) = data & 0xffff;
        ...
        /* Transfer data to the memory */
        *(uint16_t *)(temp_addr + writeaddr) = (data >> 16) & 0xffff;
...
/**
    \brief      check the EXMC SDRAM memory
    \param[in]  sdram_device: specify which a SDRAM memory block is written
    \param[out] none
    \retval     none
*/
void sdram_memcheck(uint32_t sdram_device)
{
    ...
    uint16_t ptr_read[128];

    /* Select the base address according to EXMC_Bank */
    if (sdram_device == EXMC_SDRAM_DEVICE0) {
        // temp_addr = SDRAM_DEVICE0_ADDR;
        // note: half-word data to check
        temp_size = SDRAM_DEVICE0_SIZE / sizeof(uint16_t);
    } else {
        // temp_addr = SDRAM_DEVICE1_ADDR;
        // note: half-word data to check
        temp_size = SDRAM_DEVICE1_SIZE / sizeof(uint16_t);
    }

    /* While there is data to check */
    for (; temp_size != 0;) {
        if (temp_size >= sizeof(ptr_read) / sizeof(uint16_t))
            check_size = sizeof(ptr_read) / sizeof(uint16_t);
        else
            check_size = temp_size;

        /* Transfer data to the memory */
        sdram_set_16(sdram_device, 0x00, temp_addr, check_size);
        sdram_readbuffer_16(sdram_device, ptr_read, temp_addr, check_size);
        for (uint32_t i = 0; i < check_size; i++) {
            while (0x00 != ptr_read[i])
                ;
        }

        /* Transfer data to the memory */
        sdram_set_16(sdram_device, 0xff, temp_addr, check_size);
        sdram_readbuffer_16(sdram_device, ptr_read, temp_addr, check_size);
        for (uint32_t i = 0; i < check_size; i++) {
            while (0xff != ptr_read[i])
                ;
        }
